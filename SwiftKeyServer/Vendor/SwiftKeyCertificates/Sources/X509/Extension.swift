...
extension Certificate.Extension: DERImplicitlyTaggable {
    ...
    public init(derEncoded rootNode: ASN1Node, withIdentifier identifier: ASN1Identifier) throws {
        self = try DER.sequence(rootNode, identifier: identifier) { nodes in
            ...
            // SwiftKey Android interoperability: some StrongBox implementations
            // encode KeyUsage's critical TRUE as 0x01. Accept this one-byte
            // representation only for KeyUsage. Certificate retains and verifies
            // its original tbsCertificateBytes; no signed bytes are rewritten.
            var peek = nodes
            let critical: Bool
            if let candidate = peek.next(), extensionID == [2, 5, 29, 15],
                candidate.identifier == .boolean,
                case .primitive(let bytes) = candidate.content,
                bytes.count == 1, bytes.first == 1
            {
                critical = true
                nodes = peek
            } else {
                critical = try DER.decodeDefault(&nodes, defaultValue: false)
            }
